...
static int open_soundfile(void){
  ...
  if(filename==NULL){
    if (rotateframe>0){ // always use .NUM. with rotation
      ...
      sprintf(filename,"%s%0*d.%s",filename_prefix,leading_zeros+1,0,soundfile_format);
      // sprintf(filename,"%s%0*d.%s",filename_prefix,leading_zeros,++seq,soundfile_format);
    }else{
